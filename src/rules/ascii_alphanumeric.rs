...
fn _ascii_alphanumeric(input: Option<&str>) -> bool {
...
pub fn ascii_alphanumeric_base(input: &str) -> bool {
	return _ascii_alphanumeric(file_stem(input));
}

/// ASCII alphanumeric first segment plus exactly one extension.
/// No period is also fine.
pub fn ascii_alphanumeric_both(input: &str) -> bool {
	return _ascii_alphanumeric(file_stem(input)) && _ascii_alphanumeric(file_extension(input));
}

/// All dot-separated segments must be ASCII alphanumeric.
pub fn ascii_alphanumeric_ext(input: &str) -> bool {
	return _ascii_alphanumeric(file_extension(input));
}
...
pub fn ascii_alphanumeric_builder(param: Option<String>) -> Option<RuleCheckFn> {
	...
	let checker: RuleCheckFn = match mode.as_str() {
		...
		"both" => Box::new(ascii_alphanumeric_both),
		_ => return None,
	};
...
#[cfg(test)]
mod tests {
	use super::{ascii_alphanumeric_base, ascii_alphanumeric_both, ascii_alphanumeric_ext};
	...
	fn base_accepts() {
		...
		assert!(ascii_alphanumeric_base("report."));		// not desireable, but catch in a separate rule
	...
	#[test]
	fn both_accepts() {
		assert!(ascii_alphanumeric_both("report.txt"));
		assert!(ascii_alphanumeric_both("report"));
		assert!(ascii_alphanumeric_both("ABC123.log"));
		assert!(ascii_alphanumeric_both("report."));		// not desireable, but catch in a separate rule
		assert!(ascii_alphanumeric_both(""));
	}

	#[test]
	fn both_rejects() {
		assert!(!ascii_alphanumeric_both("report.tar.gz"));
		assert!(!ascii_alphanumeric_both("my-file.txt"));
		assert!(!ascii_alphanumeric_both(".txt"));
	}
	...
	#[test]
	fn ext_rejects() {
		assert!(!ascii_alphanumeric_ext("dollars.$$$"));
		assert!(!ascii_alphanumeric_ext("middle-bad.x$x"));
		assert!(!ascii_alphanumeric_ext("middlebad.x$x"));
	}
}
